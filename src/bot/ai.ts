...
const SYSTEM_PROMPT = `You are a helpful assistant for a local service business that provides plumbing, locksmith, electrical, and handyman services.
...
export async function handleAiChat(
  ...
  const reply = await chat(SYSTEM_PROMPT, history);
...
No explanation, just the JSON.`;
  ...
  const result = await chat(SYSTEM_PROMPT, [{ role: 'user', content: prompt }]);
